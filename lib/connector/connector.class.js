...
const collectionManagerFactory = require('../collectionManager/collectionManager.classFactory');
 ...
 */
const injectDb = ({ knex, }) => {
  const CollectionManager = collectionManagerFactory({ knex, });
  ...
  class KnexConnector {
     ...
     * @param {String} sourceName The name of the table on the database
     */
    constructor({ sourceName, }) {
      this.tableName = sourceName;
    }

    /**
     * Get the collection manager bound with the connector
     * @return {CollectionManager} The collection manager bound with this class Connector
     */
    ...
    create({ items, transaction = null, }) {
      if (transaction) {
        const knexTransaction = transaction.getKnexTransaction();

        return knexTransaction(this.tableName).insert(items);
      }

      return knex(this.tableName).insert(items);
    }
